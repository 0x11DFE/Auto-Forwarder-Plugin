...
# Use OrderedDict to control the display order in the UI
...
You can specify keywords or regex patterns that messages must contain to be forwarded. This works for text messages, media captions, and now **document filenames**:
...
class AutoForwarderPlugin(dynamic_proxy(NotificationCenter.NotificationCenterDelegate), BasePlugin):
    ...
    def __init__(self):
        ...
        # Cache for preventing duplicate file forwards based on author and filename
    ...
    def _get_document_filename(self, doc):
        """Safely extracts a filename from a document's attributes."""
    ...
    def handle_message_event(self, message_object):
        ...
        # --- New Feature: Specific Author Filtering ---
        # If the rule has an author filter, check if the message author matches.
        ...
        # --- New Feature: Content-based Deduplication for Files ---
        # Prevents forwarding the same file from the same user if sent multiple times.
            ...
            if filename:
                ...
                while self.processed_files_cache:
                    ...
                    if current_time - ts > self.deduplication_window_seconds:
                        ...
                        break
                # If the file was already processed recently, skip it
    ...
    def _is_message_allowed_by_filters(self, message_object, rule):
        """
        Checks if a message should be forwarded based on the rule's granular media filters.
        This logic is refined to handle text, media, and media with captions separately.
        """
        ...
        has_media = message.media and not isinstance(message.media, (TLRPC.TL_messageMediaEmpty, TLRPC.TL_messageMediaWebPage))

        # Case 1: Text-only message (no media)
        if has_text and not has_media:
            return filters.get("text", True)

        # Case 2: Media message (with or without caption)
        if has_media:
            media_type_allowed = False
            if message_object.isPhoto() and filters.get("photos", True): media_type_allowed = True
            elif message_object.isSticker() and filters.get("stickers", True): media_type_allowed = True
            elif message_object.isVoice() and filters.get("voice", True): media_type_allowed = True
            elif message_object.isRoundVideo() and filters.get("video_messages", True): media_type_allowed = True
            elif message_object.isGif() and filters.get("gifs", True): media_type_allowed = True
            elif message_object.isMusic() and filters.get("audio", True): media_type_allowed = True
            elif message_object.isVideo() and filters.get("videos", True): media_type_allowed = True
            elif message_object.isDocument() and filters.get("documents", True): media_type_allowed = True
            
            if not media_type_allowed: return False
            # If the media type is allowed, check if its caption is also allowed (if it has one)
            if has_text:
                return filters.get("media_captions", True)
            return True # Media without a caption is allowed if its type is allowed

        # Default case (e.g., empty message, web page preview without text)
        return False
    ...
    def _process_and_send(self, message_object, event_key):
        ...
        if keyword_pattern:
            ...
            # If it's a document, append its filename to the text being checked
            if message_object.isDocument():
                doc = getattr(message.media, 'document', None)
                filename = self._get_document_filename(doc)
                if filename:
        ...
        self._send_forwarded_message(message_object, event_key)
    ...
    def _send_album(self, message_objects, rule):
        ...
        try:
            # Check keywords against all text content in the album before proceeding
            if keyword_pattern:
                full_text_to_check = ""
            ...
            # Only attach a caption if the "Media Captions" filter is enabled
    ...
    def _send_forwarded_message(self, message_object, rule):
        ...
        try:
            ...
            original_text = ""
            # Logic to decide if text/caption should be included based on new filters
            if has_text:
                filters = rule.get("filters", {})
    ...
    def _load_forwarding_rules(self):
        ...
        except Exception: self.forwarding_rules = {}
    ...
    def _on_menu_item_click(self, context):
        ...
        # Show modification dialog if rule exists, otherwise show creation dialog
    ...
    def _show_destination_input_dialog(self, source_id, source_name, existing_rule=None):
        ...
        try:
            ...
            # --- Standard Inputs: Destination and Keywords ---
            ...
            # --- General Checkboxes: Copy Mode and Replies ---
            ...
            # --- Divider ---
            divider_params = LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, 1) # 1px height
            vertical_margin_px = int(TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 12, activity.getResources().getDisplayMetrics()))
            ...
            author_filter_params.setMargins(margin_px, 0, margin_px, margin_px // 2)
            author_filter_input.setHint("Filter by Author ID/@user (optional, CSV)")
            author_filter_input.setTextColor(Theme.getColor(Theme.key_dialogTextBlack))
            author_filter_input.setHintTextColor(Theme.getColor(Theme.key_dialogTextHint))
            author_filter_input.setLayoutParams(author_filter_params)
            main_layout.addView(author_filter_input)
            ...
            # --- Dynamic UI Logic for Author Filter Input ---
            # Shows the specific author filter only when "Users" or "Bots" is checked.
            ...
            forward_users_checkbox.setOnCheckedChangeListener(listener)
            forward_bots_checkbox.setOnCheckedChangeListener(listener)
            ...
            # --- Set Initial State of All UI Elements ---
            if existing_rule:
                ...
                author_filter_input.setText(existing_rule.get("author_filter", ""))
            ...
            else: # Set defaults for a new rule
            ...
            # Set initial visibility of the author filter input
            ...
            def on_set_click(b, w):
                ...
                self._process_destination_input(
                    ...
                    author_filter_input.getText().toString(),
    ...
    def _process_destination_input(self, source_id, source_name, user_input, keyword_pattern, author_filter, drop_author, quote_replies, forward_users, forward_bots, forward_outgoing, filter_settings):
        """Handles all destination types and collects all rule settings."""
        cleaned_input = (user_input or "").strip()
        if not cleaned_input: return

        rule_settings = {
            "keyword_pattern": keyword_pattern, "author_filter": author_filter, "drop_author": drop_author, 
            "quote_replies": quote_replies, "forward_users": forward_users, "forward_bots": forward_bots, 
        ...
        except ValueError: self._resolve_as_username(cleaned_input, source_id, source_name, rule_settings)
    ...
    def _finalize_rule(self, source_id, source_name, destination_id, dest_name, rule_settings):
        ...
        if destination_id == 0:
            ...
            return
        rule_data = {
            "destination": destination_id, "enabled": True,
